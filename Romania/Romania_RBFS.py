...
# creates a Map object that represents the country of Romania
countryName = "Romania"
romaniaMap = Map(countryName, -1, genVerbose, "./Romania/romania.csv")

# loads the adjacency data from the json file
with open('./Romania/data.json') as file:
    data = json.load(file)
...
# problem: a problem that can be solved with a recursive best first search, in this case, a graph that represents cities locations and distances between them, specifically from Romania
romaniaProblem = MapProblem(State(origin), State(goal), adjacencyData, romaniaMap)
...
def RecursiveBestFirstSearch(problem : MapProblem, hFunc = None) -> str:
    ...
    romaniaMap.updateState(initialNode.state.name, "start")
    ...
    romaniaMap.updateState(rbfsResult[0].state.name, "goal")
...
def RBFS(problem : MapProblem, node : Node, fLimit) -> Node:
    ...
    for succesor in successors:
        romaniaMap.updateState(succesor.state.name, "frontier")

        succesor.fCost = max(succesor.pathCost + hCostFunction(succesor), node.fCost)
    ...
    while True:
        ...
        ### Updates Visuals
        romaniaMap.updateState(best.state.name, "open")
        romaniaMap.maxPath = best.pathCost
        romaniaMap.currentVisitedNode = best.state.name
...
def playVisualization(sleepTime = 2):
        ...
        while running:
            ...
            currentVisitedNode = font.render("Visited Node: " + str(romaniaMap.currentVisitedNode), True, black)
...
resultMexico1Node = run_RBFS_as_thread(romaniaProblem, None)
...
resultsToShow = [
    ...
    [resultMexico1Node, f"Results Romania {origin} to {goal}"]
    ]
